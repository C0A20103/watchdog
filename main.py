...
def main():
    ...
    # カメラ初期設定
    cascade = cv2.CascadeClassifier(os.getcwd()+os.sep+'haarcascade_frontalface_alt2.xml')
    ...
    # 録画初期設定
    event = Event()
    thread = Thread(target=recording)
    thread.start()
    ...
    while True:
        ...
        fps = mainFps if(0 < sleepTime) else 1/((1/mainFps)-sleepTime)
